...
def messages_for_query2():
    messages = []
    for i in range(30):
        msg = Message(BOOK_MSG_TYPE, title=str(i), year=1950+5*i)
        if i%2:
            msg.authors = ['Author1']  
        messages.append(msg)
    return messages
...
def main():
    ...
    com.publish_message_next_routing_key('2.0' ,Batch(messages).to_bytes())
    print("Eof")
    com.publish_message_next_routing_key('2.0' ,Batch([]).to_bytes())
    
    #com = Communicator()
    batch_bytes = com.receive_subscribed_message(GATEWAY_EXCHANGE_NAME)
    batch = Batch.from_bytes(batch_bytes)
    print("Recibi de resultado eto: ", batch.messages[0].authors)
